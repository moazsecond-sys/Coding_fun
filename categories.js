    ...
    <div class="card" style="border-top:3px solid ${config.color}">
      ...
      <div style="margin-top:20px;border-top:1px solid #333;padding-top:15px">
        ...
        <div id="comments-${p.id}" style="max-height:300px;overflow-y:auto;margin-bottom:10px">
          ${comments.map(c=>`
            <div style="display:flex;gap:8px;margin-bottom:10px">
              <div style="width:32px;height:32px;border-radius:50%;background:${config.color};color:#000;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:14px">
              ...
              <div style="flex:1;background:#2a2a2a;padding:8px 12px;border-radius:18px">
                ...
                <span style="font-size:11px;color:#666">${timeAgo(c.created_at || p.created_at)}</span>
              </div>
            </div>
        ...
        <div style="display:flex;gap:8px">
          <div style="width:32px;height:32px;border-radius:50%;background:#4ade80;color:#000;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:14px">
            ${document.getElementById('userEmail').textContent.charAt(0).toUpperCase()}
          </div>
...
async function addComment(project_id){
  ...
  input.value = '';
  loadProjects(); // نحدث عشان يجيب التعليق الجديد
}
